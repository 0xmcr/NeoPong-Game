# AeroCut AI by MCR Provider Guide

Everything you need to know about every provider in AeroCut AI by MCR — setup instructions, pricing, free tiers, and what each unlocks.
...
AeroCut AI by MCR now uses those published rates in the Grok tool estimators.
...
AeroCut AI by MCR uses the new-console API key flow:
...
Doubao Speech 2.0 is billed by character package or usage in Volcengine. AeroCut AI by MCR estimates cost from text length and prefers provider-returned usage metadata when available.
...
The CLI is consumed as `npx hyperframes`. Do not use `npx @hyperframes/cli`; that package name is not the AeroCut AI by MCR runtime path.
...
HyperFrames workspaces live under `projects/<project-name>/hyperframes/`. Final videos still follow the normal AeroCut AI by MCR convention: `projects/<project-name>/renders/final.mp4`.
