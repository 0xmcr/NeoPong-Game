...
│
├── skills/                 # Layer 2: AeroCut AI by MCR-specific agent instructions
...
AeroCut AI by MCR does not call LLM APIs at runtime. The coding assistant running in the user's IDE _is_ the LLM. Tools that need generation (images, video, TTS) call domain-specific APIs directly (ElevenLabs, fal.ai, HeyGen, etc.), not general-purpose LLM endpoints.
...
orchestration:
  mode: mcr-executive-producer
  skill: pipelines/explainer/mcr-executive-producer
...
stages:
  - name: research
    skill: pipelines/explainer/mcr-research-director
...
Layer 3: .agents/skills/          External technology knowledge (47 skills)
         "How the technology works"    FFmpeg, ElevenLabs API, FLUX, Remotion, Three.js, etc.
              ...
              |
Layer 2: skills/                  AeroCut AI by MCR conventions
...
AeroCut AI by MCR has a multi-runtime composition layer. Three engines live behind `video_compose`, chosen at proposal and locked in `edit_decisions.render_runtime`:
