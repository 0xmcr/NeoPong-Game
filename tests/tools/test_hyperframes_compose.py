...
def test_animation_proposal_director_has_no_hardcoded_costs_or_keys():
    ...
    and specific API key names in the animation mcr-proposal-director. Uses a
    ...
    body = (
        Path(__file__).resolve().parent.parent.parent
        / "skills" / "pipelines" / "animation" / "mcr-proposal-director.md"
    ).read_text(encoding="utf-8")
    ...
    if violations:
        ...
        raise AssertionError(
            ...
            f"animation/mcr-proposal-director.md. Director skills must pull these "
    ...
    assert "provider_menu_summary" in body or "estimate_cost" in body, (
        "Animation mcr-proposal-director must reference provider_menu_summary() "
