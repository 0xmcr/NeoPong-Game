...
>
> The rule applies to script narration AND to the metadata fields mcr-scene-director consumes. For the universal vocabulary that names these visual primitives, see `skills/creative/video-gen-prompting.md`.

## Subject Transitions in the Script

When a script beat introduces a new subject, kills one off, or hands focus from one subject to another, **name the transition explicitly** so the mcr-scene-director doesn't have to infer it. The CMU/Harvard taxonomy uses four labels:
...
When writing a beat, attach one line of camera intent so the mcr-scene-director doesn't have to invent it from a blank slate. Use the universal vocabulary in `skills/creative/video-gen-prompting.md` (Subject / Subject Motion / Scene / Spatial Framing / Camera). One line is enough — the mcr-scene-director will expand it.
...
The camera-intent line is consumed verbatim by the mcr-scene-director's 5-aspect spec — keep it concrete, no mood adjectives.
