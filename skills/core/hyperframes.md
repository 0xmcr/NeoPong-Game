...
This is the **AeroCut AI by MCR-specific** guide to HyperFrames. It explains when
AeroCut AI by MCR pipelines should choose HyperFrames over Remotion, how AeroCut AI by MCR
...
## When AeroCut AI by MCR should pick HyperFrames (vs Remotion vs FFmpeg)

AeroCut AI by MCR separates two concepts:
...
| Synthetic UI / fake terminal / fake browser demo | Either — depends on existing coverage | AeroCut AI by MCR already ships Remotion `TerminalScene` (see `synthetic-screen-recording` Layer 3). For UI chrome beyond terminal, HyperFrames HTML is easier. |
...
AeroCut AI by MCR artifacts into HyperFrames project files:

| AeroCut AI by MCR artifact field | HyperFrames target |
|---|---|
...
`STORYBOARD.md` as step-by-step workspace files. AeroCut AI by MCR does **not**
...
AeroCut AI by MCR playbooks currently translate into Remotion `themeConfig`
...
- ❌ Running `hyperframes init` from the AeroCut AI by MCR orchestrator. `init`
...
Proposal and mcr mcr compose directors for adopted pipelines describe runtime choice
explicitly — see each pipeline's `mcr-proposal-director.md` and
`mcr-compose-director.md`.
