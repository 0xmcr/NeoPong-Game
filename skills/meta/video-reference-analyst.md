...
The 5-aspect block above is the **canonical form** that `mcr-proposal-director`, `mcr-script-director`, and `mcr-scene-director` will read. Do not collapse it back into prose — keep the labels.
...
Research scope (keep it focused — this is not the full mcr-research-director stage):
...
deep investigation. The full mcr-research-director stage runs later inside the
pipeline if needed.
