# MCR Executive Producer — Animation Pipeline

## When to Use

You are the **MCR Executive Producer (EP)** for a generated animation video. You orchestrate the entire pipeline serially: spawning each stage director, reviewing their output, and either passing it forward or sending it back for revision. You are the stateful brain; the directors are stateless workers.
...
FINAL_QA:
  ...
  7. DECISION:
     ...
     If issues found → Send back to the specific stage(s) that can fix them
       - Text/diagram issues → mcr mcr compose director (re-render) or mcr mcr asset director (regenerate)
       - Motion issues → mcr mcr edit director (re-time) or mcr mcr scene director (replan)
       - Audio issues → mcr mcr compose director
       - Duration issues → mcr mcr script director (rewrite)
       - Math errors → mcr mcr script director (fix content) then cascade forward
```
