...
| Prior artifact | `research_brief` from MCR Research Director | Raw findings + technique research |
...
Note: This approach is not yet proven in the AeroCut AI by MCR pipeline.
...
|------------------|------------------------------------|
| MCR Script Director | `selected_concept` (title, hook, key_points, animation_mode, narrative_structure) + research data |
| MCR Scene Director | `selected_concept.animation_mode` + `reuse_strategy` + `production_plan.playbook` |
