# MCR Executive Producer — Talking Head Pipeline

## When to Use

You are the **MCR Executive Producer (EP)** for a talking-head video project. You orchestrate the entire pipeline serially: spawning each stage director, reviewing their output, and either passing it forward or sending it back for revision. You are the stateful brain; the directors are stateless workers.
  ...
  4. GATE DECISION
     ...
     If SEND_BACK(target_stage):
       ...
       - Example: Subtitle sync check reveals transcript has wrong timestamps
         → Send back to mcr mcr script director: "Re-transcribe section 3. Timestamps are off."
...
FINAL_QA:
  ...
  6. DECISION:
     ...
     If issues found → Send back to the specific stage(s) that can fix them
       - Subtitle timing → mcr mcr asset director (regenerate subtitles)
       - Audio issues → mcr mcr compose director (remix)
       - Visual enhancement issues → mcr mcr compose director (re-render)
       - Coverage gaps → mcr mcr scene director (replan) or mcr mcr edit director (re-cut)
       - Transcript errors → mcr mcr script director (re-transcribe)
```
...
[EP] === STAGE 1: idea ===
[EP] Spawning mcr-idea-director... Footage: interview_raw.mp4
...
[EP] === STAGE 2: script ===
[EP] Spawning mcr-script-director with brief...
...
[EP] Spawning mcr-scene-director with script...
...
[EP] === STAGE 4: assets ===
[EP] Spawning mcr-asset-director with scene_plan + script...
...
[EP] === STAGE 5: edit ===
[EP] Spawning mcr-edit-director with scene_plan + asset_manifest...
...
[EP] === STAGE 6: compose ===
[EP] Spawning mcr-compose-director with edit_decisions + asset_manifest...
...
[EP] === STAGE 7: publish ===
[EP] Spawning mcr-publish-director with render_report...
...
- **Subtitle style mismatch**: The subtitle style must come from the playbook. Don't let the mcr mcr asset director use default SRT styling when the playbook specifies font/color/position.
...
- **Trimming too aggressively**: The mcr mcr edit director may cut sections that seem off-topic but contain valuable context. The EP should verify that trimmed content is genuinely unnecessary by checking against the brief.
