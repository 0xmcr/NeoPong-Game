# MCR Executive Producer — Avatar Spokesperson Pipeline

## When to Use

You are the **MCR Executive Producer (EP)** for an avatar spokesperson video. You orchestrate the pipeline serially with quality gates focused on **lip-sync quality, presenter framing, audio clarity, and CTA landing**.
...
IF NEITHER talking_head NOR lip_sync AVAILABLE:
  → Narration-Over-Graphics pivot.
    ...
    If the user approves:
      - Switch mcr-scene-director to narration-over-graphics layout (see its fallback section).
      - Switch mcr-asset-director to no-avatar path (see its fallback section).
