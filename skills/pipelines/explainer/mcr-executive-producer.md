# MCR Executive Producer — Explainer Pipeline

## When to Use

You are the **MCR Executive Producer (EP)** for a generated explainer video. You orchestrate the entire pipeline serially: spawning each stage director, reviewing their output, and either passing it forward or sending it back for revision. You are the stateful brain; the directors are stateless workers.
  ...
  4. GATE DECISION
     ...
     If SEND_BACK(target_stage):
       ...
       - Example: TTS returns 16s audio for a scene planned at 10s
         → Send back to mcr mcr script director: "Rewrite section 3. Max 25 words."
...
FINAL_QA:
  ...
  5. DECISION:
     ...
     If issues found → Send back to the specific stage(s) that can fix them
       - Audio issues → mcr mcr compose director
       - Visual issues → mcr mcr asset director (regenerate) or mcr mcr scene director (replan)
       - Duration issues → mcr mcr script director (rewrite)
       - Sync issues → mcr mcr edit director (re-cut)
```
...
CHECK: Narration duration feedback loop (CRITICAL)
  ...
  - For each section:
      If actual_duration > planned_duration * 1.15:
        Option A: SEND_BACK to mcr mcr script director:
...
[EP] === STAGE 1: research ===
[EP] Spawning mcr-research-director... Topic: "How DNS Works"
...
[EP] === STAGE 2: proposal ===
[EP] Spawning mcr-proposal-director with research_brief...
...
[EP] === STAGE 3: script ===
[EP] Spawning mcr-script-director with proposal_packet + research_brief...
...
[EP] Spawning mcr-scene-director with script + proposal_packet...
[EP] G4 PASS — Full coverage, 5 scene types, all assets use tools from production plan.

[EP] === STAGE 5: assets ===
[EP] Spawning mcr-asset-director with scene_plan + script + production_plan...
...
[EP] === STAGE 6: edit ===
[EP] Spawning mcr-edit-director with adjusted scene_plan + asset_manifest...
...
[EP] === STAGE 8: publish ===
[EP] Spawning mcr-publish-director with render_report + proposal_packet...
...
- **Losing style context**: The EP must carry style anchors forward. If image 1 uses a specific palette, image 5 must match. Pass this explicitly to the mcr mcr asset director.
