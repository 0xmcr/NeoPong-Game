# MCR Scene Director - Documentary Montage Pipeline
...
The mcr mcr scene director's job is NOT "pick clips". It is "describe what the
...
If `era_mix = "any"`, mix sources per slot — the mcr mcr scene director
...
Don't over-specify. The mcr mcr asset director's job is to rank candidates
against your description. If you nail down the description AND the
exact clip, you've done the mcr mcr asset director's job badly and pre-empted
...
array is what the mcr mcr asset director actually reads — it carries the
...
This is the artifact the mcr mcr asset director will run retrieval against.
