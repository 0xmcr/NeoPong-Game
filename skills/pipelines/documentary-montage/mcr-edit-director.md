# MCR Edit Director - Documentary Montage Pipeline
...
This is where documentary technique lives. If the mcr mcr asset director did
...
- The brief approved a music track that the mcr mcr edit director now wants
...
table from the mcr mcr scene director's tone chart:
...
The mcr mcr scene director gave you a slot order. That order is the intent.
...
- The final image isn't landing. The last 5-10s carries
  disproportionate weight; if the mcr mcr scene director's choice dies, move
...
time. The mcr mcr edit director's job is to decide **when** the tag appears.
...
If `mode == "concat"`, omit this section — the mcr-compose-director will
