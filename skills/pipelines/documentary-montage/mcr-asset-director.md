# MCR Asset Director - Documentary Montage Pipeline
...
1. **Build the corpus** — fan the mcr mcr scene director's queries out across
...
2. **Query rewriting.** The mcr mcr scene director will have already rewritten
...
mcr mcr scene director adds slots later, you can grow the same corpus instead
...
- Use the slot **description**, not the queries. The description is
  the rich noun-and-adjective string the mcr mcr scene director wrote. CLIP
...
- **Motion fit.** The tone table from the mcr mcr scene director tells you
...
score above 0.22, tell the mcr idea director the slot is unfilmable from
...
Read `brief.music_plan`. Execute exactly the plan the mcr idea director
...
The `rejected_picks` log matters. The mcr mcr edit director reads it when a
...
Used when the mcr mcr edit director wants to confirm the provider/URL before
locking the cut.
